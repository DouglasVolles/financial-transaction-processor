Feature: Account creation service
  ...
  Scenario: Create first account successfully
    Given a clean account database
    And a valid account request with customer cpfcnpj "123.456.789-01", available balance 1000.50, reserved balance 125.25, credit limit 5000.00 and status "Active"
    When I create the account
    Then the account creation status should be "Created"
    And the created account id should be 1
    And one account should be persisted with id 1, customer id 101, identification "ACC-001", available balance 1000.50, reserved balance 125.25, credit limit 5000.00 and status "Active"
