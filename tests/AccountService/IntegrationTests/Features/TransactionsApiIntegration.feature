Feature: Transactions API integration
  ...
  Scenario: Get all transactions returns empty list
    ...
    When I send GET request for transactions to "/api/financialtransaction/transactions"
  ...
  Scenario: Add transaction publishes message to transactions queue
    ...
    When I send POST request to "/api/financialtransaction/transactions" with operation "credit", account id "ACC-001", amount 10000, currency "BRL", reference id "TXN-001" and metadata "{\"description\":\"Deposito inicial\"}"
    Then the transaction response status code should be 200
  ...
  Scenario: Add transaction with duplicated reference id returns existing record
    ...
    When I send POST request to "/api/financialtransaction/transactions" with operation "debit", account id "ACC-001", amount 5000, currency "BRL", reference id "TXN-REF-1" and metadata "{\"description\":\"Duplicate\"}"
    Then the transaction response status code should be 200
